...
public class Device {
    ...
    /**
     * Android 10+ 限制访问 Build.SERIAL
     * 使用 Build.UNKNOWN 作为替代
     */
    ...
    static {
        ...
        // Android 10+ 无法获取真实序列号，使用 ANDROID_ID 替代
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.Q) {
            serial = Build.UNKNOWN; // Android 10+ 返回 "unknown"
        } else {
            serial = Build.SERIAL;
        }
    }
    ...
    /**
     * 获取 MAC 地址
     * Android 10+ 限制访问 WiFi MAC 地址，getMacAddress() 将返回 null 或固定值
     * 此方法尝试多种方式获取，但 Android 10+ 可能无法获取真实 MAC
     * 
     * @return MAC 地址，如果无法获取则返回 null
     */
    @SuppressLint("HardwareIds")
    public String getMacAddress() throws Exception {
        // Android 10+ 不再允许第三方应用获取 MAC 地址
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.Q) {

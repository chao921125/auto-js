 ...
 * 
 * Android 10+ 优化：
 * - 前台服务类型声明为 mediaProjection
 * - 添加投射通知（Android 10 强制要求）
 * - 优化 ImageReader 异步处理
 */
public class ScreenCapturer {
    ...
    private void initVirtualDisplay(int width, int height, int screenDensity) {
        ...
        // Android 10+ 使用 VIRTUAL_DISPLAY_FLAG_SUPPORTS_TOUCH 提升性能
        int flags = DisplayManager.VIRTUAL_DISPLAY_FLAG_AUTO_MIRROR;
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
            flags |= DisplayManager.VIRTUAL_DISPLAY_FLAG_SUPPORTS_TOUCH;
        }
        
        mVirtualDisplay = mMediaProjection.createVirtualDisplay(LOG_TAG,
                width, height, screenDensity, flags,
